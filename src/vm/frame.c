...
evict_page(void)
{
    ...
    while(1)
    {
        // if(clock_hand == NULL || clock_hand == list_tail(&frame_table))
        ...
        else //accessed bit 0
        {
            ...
            // is_dirty = pagedir_is_dirty(cur->thread->pagedir, cur->page->vaddr); //아마 오류날수도 있음. page일지 kaddr일지 생각해보자.
